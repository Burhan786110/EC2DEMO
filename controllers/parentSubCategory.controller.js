...
exports.poststatuschange = async (req, res, next) => {
  try {
    const { parentSubCategoryID, status } = req.body;

    let delete_status;
    let result = await sequelize.transaction(async (t) => {
      if (status === "1") {
        delete_status = await ParentSubCategory.update(
          { status: "1" },
          {
            where: { parentSubCategoryID: parentSubCategoryID, status: ["1", "0"] },
            transaction: t,
          }
        );
      } else {
        ...
        if (associatedProducts) {
          return res.status(200).json({
            status: 401,
            message:
              "You cannot deactivate this category. It is already associated with Product.",
          });
        } else {
          delete_status = await ParentSubCategory.update(
            { status: "0" },
      ...
      const message =
        status == 1
          ? "Parent Category is activated successfully."
          : "Parent Category is deativated successfully.";
